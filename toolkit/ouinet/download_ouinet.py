...
def unpackage_file(zfile, output_dir):
    ...
    with zipfile.ZipFile(zfile, "r") as ouinet_zip:
        for member in ouinet_zip.namelist():
            ...
            if member.startswith("ouinet-windows-x64-v1.6.2-dev0/") and not member.endswith("/"):
                # Extract the file to the output directory, stripping 'build/windows/'
                relative_path = os.path.relpath(member, "ouinet-windows-x64-v1.6.2-dev0")
...
def main():
    ...
    ouinet_url = (
        # @TODO: update url - https://gitlab.com/equalitie/ouinet/-/merge_requests/153
        "https://gitlab.com/equalitie/ouinet/-/package_files/256059766/download"
    )
